...
class QNetwork(nn.Module):
    def __init__(self, obs_dim, act_dim, hidden_size=256, d2rl=True):
        super().__init__()

        # build networks
        if not d2rl:
            self.qnetwork_local = NAF(obs_dim, act_dim, hidden_size)
            self.qnetwork_target = NAF(obs_dim, act_dim, hidden_size)
        else:
            self.qnetwork_local = DeepNAF(obs_dim, act_dim, hidden_size)
            self.qnetwork_target = DeepNAF(obs_dim, act_dim, hidden_size)
